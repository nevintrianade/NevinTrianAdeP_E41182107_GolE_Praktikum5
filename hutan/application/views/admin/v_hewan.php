...
<div class="panel-heading">
	...
	<center><?php echo anchor('admin/hewan/tambah','Tambah Data'); ?></center> <!-- meneruskan input form ke direktori file crud/tambah dengan method post  -->
	<table class="table table-bordered">  <!-- membuat tabel  -->
		<tr>
			...
			<th>DESKRIPSI HEWAN</th> <!-- membuat deskripsi  -->
			<th>JUMLAH_JANTAN</th> <!-- membuat jumah jantan  -->
			<th>JUMLAH_BETINA</th> <!-- membuat jumlah betina  -->
            <th>GAMBAR</th> <!-- membuat label gambar  -->
            <th>AKSI</th> <!-- membuat label aksi  -->
		</tr>
		<?php 
		$no = 1;
		foreach($hewan as $h){ //mengubah $user menjadi $u
		?>
		<tr>
			...
			<td><img src="<?php echo base_url('upload/hewan/'.$h->GAMBAR) ?>" width="64" /></td> <!-- menampilkan gambar  -->
			<td>
				
			    <?php echo anchor('admin/hewan/edit/'.$h->KD_HEWAN,'Edit'); ?> <!-- menampilkan button edit dan fungsinya -->
                <?php echo anchor('admin/hewan/hapus/'.$h->KD_HEWAN,'Hapus'); ?> <!-- menampilkan button hapus dan fungsinya -->
			</td>
		</tr>
		<?php } ?>
    </table>
